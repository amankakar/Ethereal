...
contract Ethereal is Ownable, Pausable, ERC721URIStorage, ERC721Holder, ReentrancyGuard {
    ...
    function updateGem(uint256 _id, uint256 _collection, uint256 _denomination, uint256 _redeemFee, bool _active)
        ...
        onlyOwner
    {
        // @audit : _denomination check missing <= 1e4
        gems[_id].collection = _collection; // this got updated 1 to 2
    ...
    function _mintWstEth(uint256 _id, uint256 _collection, address _recipient) internal returns (uint256 tokenId_) {
        tokenId_ = ++_tokenCounter;
        _safeMint(_recipient, tokenId_); // @audit : reentrancy here
        circulatingGems++;
        ...
        metadata[tokenId_] = Metadata(IwstETH(wstETH).balanceOf(address(this)) - preBalance, _collection, _id); // collection : 1 , gem : 1
        emit GemMinted(tokenId_, _recipient, msg.value);
    }
    ...
    function _redeemEth(uint256 _tokenId) internal {
        ...
        _burn(_tokenId);
        circulatingGems--;
        uint256 redeemFee = (metadata[_tokenId].balance * gems[metadata[_tokenId].gem].redeemFee) / 1e4; // @audit : change in fee will end up receiving less return
